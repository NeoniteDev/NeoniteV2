...
app.post("/fortnite/api/game/v2/profile/:accountId/client/:command", async (req, res, next) => {
	...
	switch (command) {
		...
		case "PurchaseCatalogEntry": {
			checkValidProfileID("common_core", "athena")
			...
			const commonCoreProfile = getOrCreateProfile("common_core");
			const lootResult = [];

			for (itemGrant of catalogEntryToPurchase.itemGrants) {
				if (!grantProfile.profileData.items[itemGrant.templateId]) {
					lootResult.push({
			...
			// Removes vbucks from profile
			try {
				commonCoreProfile.profileData.items["Currency:MtxPurchased"].quantity = commonCoreProfile.profileData.items["Currency:MtxPurchased"].quantity - req.body.expectedTotalPrice;
				Profile.saveProfile(accountId, "common_core", commonCoreProfile.profileData);
				Profile.bumpRvn(commonCoreProfile.profileData);
				Profile.saveProfile(accountId, "common_core", commonCoreProfile.profileData);
			} catch {}
			...
			athenaProfile.response.profileChanges = [{
					...
					changeType: "fullProfileUpdate",
					profile: commonCoreProfile.profileData
				}]

			response.multiUpdate = [athenaProfile.response, commonCoreProfile.response];
			break;
		}
