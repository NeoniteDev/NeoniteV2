...
module.exports = (app) => {
    ...
    app.get('/lightswitch/api/service/bulk/status', (req, res) => {
        const serviceId = (req.query.serviceId || 'fortnite').toLowerCase();
        const response = {
    ...
    app.get('/lightswitch/api/service/:serviceId/status', (req, res) => {
        ...
        const response = {
            serviceInstanceId: serviceId.toLowerCase(),
            status: 'UP',
            message: 'Hello',
            maintenanceUri: 'https://dsc.gg/neonite',
            allowedActions: [],
    ...
    app.get("/launcher/api/public/assets/:platform/:catalogItemId/:appName", async (req, res) => {
        ...
        axios.get("https://launcher-public-service-prod06.ol.epicgames.com" + req.url, { headers: { authorization: "bearer " + token } })
            .then(response => res.json(response.data))
            .catch(async (error) => {
                if (error.response.data.errorCode == "errors.com.epicgames.launcher.download_info_not_found" && req.query.manifest) {
                    const BuildVersion = req.headers["user-agent"].split("/")[1].replace(/ /g, "-") || "";
                    await axios.head(`https://raw.githubusercontent.com/VastBlast/FortniteManifestArchive/main/Fortnite/Android/${req.query.manifest}.manifest`);
                    res.json({
    ...
    app.get("/launcher/api/public/distributionpoints/", (req, res) => {
        ...
        const distributions = GithubManifestUserAgent.includes(FormattedUserAgent) ? [
    ...
    app.all("/fortnite/api/game/v2/matchmakingservice/ticket/player/:accountId", (req, res) => {
        ...
        const [NetCL, HotfixVerion, Region, Playlist] = bucketId.split(':');
        if (!NetCL || !Region || !Playlist || HotfixVerion === -1) {
            throw new ApiException(errors.com.epicgames.fortnite.invalid_bucket_id).withMessage(`Failed to parse bucketId: '${bucketId}'`).with(bucketId);
        }
        ...
        const data = {
            ...
            "bucketId": `FN:Live:${NetCL}:${HotfixVerion}:${Region}:${Playlist}:PC:public:1`,
            "attributes": {
